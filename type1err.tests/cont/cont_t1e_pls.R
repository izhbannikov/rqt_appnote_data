...
library(rqt)
library(SKAT)
# how to run: source("/Users/ilya/Dropbox/rqt/AppNote/MinorRevision/tests/type1err.tests/cont/cont_t1e_pls.R")

proj.dir <- "/Users/ilya/Projects/rqt.dev/tests/type1error.tests/cont_pls/" # project directory, change it if you need.
n.snp <- 50
...
pval.table.rqt <- data.frame(matrix(nrow=n, ncol=3*length(alpha.level)))
pval.table.qt <- data.frame(matrix(nrow=n, ncol=3*length(alpha.level)))
pval.table.skat <- data.frame(matrix(nrow=n, ncol=length(alpha.level)))
pval.table.skato <- data.frame(matrix(nrow=n, ncol=length(alpha.level)))
...
for(alpha in alpha.level) { 
    ...
    for (i in 1:n){
        ...
        system(paste("python /Users/ilya/Projects/cophesim_stable/cophesim.py -c -i ", proj.dir, "sim.plink -o ", proj.dir, "testout" , " >/dev/null", sep=''))
        ...
        if(!is.na(results(res)$pValue$pVal1)) {
          ...
          pval.table.rqt[i,3*(indx-1)+1] <- results(res)$pValue$pVal1
        } else {
        ...
        if(!is.na(results(res)$pValue$pVal2)) {
          ...
          pval.table.rqt[i,3*(indx-1)+2] <- results(res)$pValue$pVal2
        } else {
        ...
        if(!is.na(results(res)$pValue$pVal3)) {
          ...
          pval.table.rqt[i,3*(indx-1)+3] <- results(res)$pValue$pVal3
        } else {
        ...
        if(!is.na(results(res.qt)$pValue$pVal1)) {
          ...
          pval.table.qt[i,3*(indx-1)+1] <- results(res.qt)$pValue$pVal1
        } else {
        ...
        if(!is.na(results(res.qt)$pValue$pVal2)) {
          ...
          pval.table.qt[i,3*(indx-1)+2] <- results(res.qt)$pValue$pVal2
        } else {
        ...
        if(!is.na(results(res.qt)$pValue$pVal3)) {
          ...
          pval.table.qt[i,3*(indx-1)+3] <- results(res.qt)$pValue$pVal3
        } else {
        ...
        pval.table.skat[i,indx] <- res.skat
        pval.table.skato[i,indx] <- res.skat.o
...
write.table(x = pval.table.rqt, file = paste(proj.dir, "pval.table.rqt.txt", sep=""), row.names = F, quote=F)
write.table(x = pval.table.qt, file = paste(proj.dir, "pval.table.qt.txt", sep=""), row.names = F, quote=F)
write.table(x = pval.table.skat, file = paste(proj.dir, "pval.table.skat.txt", sep=""), row.names = F, quote=F)
write.table(x = pval.table.skato, file = paste(proj.dir, "pval.table.skato.txt", sep=""), row.names = F, quote=F)
