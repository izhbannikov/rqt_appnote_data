# Power test for rqt LASSO, presence of LD#
library(rqt)
library(SKAT)
# how to run: source("~/Dropbox/rqt/AppNote/MajorRevision/tests/power.tests/power.ld.cont/power.ld.cont_lasso.R")

proj.dir <- "/Users/ilya/Projects/rqt.dev/tests/power.tests/power.ld.cont_lasso/" # project directory, change it if you need.
n.snp <- 50
...
for(s in caus.var) {
    ...
    for (i in 1:n){
        ...
        res <- geneTest(obj, method="lasso", out.type = "C", cumvar.threshold = 95, weight=F)
