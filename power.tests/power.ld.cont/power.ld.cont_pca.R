...
proj.dir <- "/Users/ilya/Projects/rqt.dev/tests/power.tests/power.ld.cont_pca/" # project directory, change it if you need.
n.snp <- 50
...
for(s in caus.var) {
    ...
    eff <- sapply(0:(n.caus-1), function(n) { paste(n, ':', ifelse( (runif(1) > 0.5), -0.1, 0.1 ), sep='') })
    ...
    for (i in 1:n){
        ...
        res <- geneTest(obj, method="pca", out.type = "C", cumvar.threshold = 90, weight=F)
