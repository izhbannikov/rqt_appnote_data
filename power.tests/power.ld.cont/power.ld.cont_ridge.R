# Power test for rqt ridge reg, presence of LD#
library(rqt)
library(SKAT)
# how to run: source("~/Dropbox/rqt/AppNote/MajorRevision/tests/power.tests/power.ld.cont/power.ld.cont_ridge.R")

proj.dir <- "/Users/ilya/Projects/rqt.dev/tests/power.tests/power.ld.cont_ridge/" # project directory, change it if you need.
n.snp <- 50
...
for(s in caus.var) {
    ...
    for (i in 1:n){
        ...
        res <- geneTest(obj, method="ridge", out.type = "C", cumvar.threshold = 95, weight=F)
        ...
        res.pow$var.pool.qt <- res.pow$var.pool.qt + results(res)$var.pooled
